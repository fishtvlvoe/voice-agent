...
- [FACT] `tasks.md` 目前有 7 個未勾選、9 個已勾選任務；未勾選項目集中在 LINE secrets、知識查詢行為與真人驗收。（證據：`rg -c "^- \\[ \\]"`、`rg -c "^- \\[x\\]"`）
...
- [VERIFIED] 已部署 `voice-agent`，最新驗證版本 `6c76dfb9-232e-46c6-9fe8-cfb5c7152278`；遠端知識匯入 smoke test 回傳 `ok:true` 與 `chunkCount:1`，等待非同步索引後用 `wrangler vectorize query --vector-id` 查回同一 vector，score `0.9999984`；D1、Vectorize 測試資料與臨時 secret 已清除。
- [UNVERIFIED] LINE secrets、實際 LINE Webhook、真人回覆、知識查詢的完整相關性／無資料行為尚未驗證。

## 進行中的 change

- [TASK] `line-chat-memory-knowledge` 規劃 LINE 文字 webhook、歷史記錄查詢與 Vectorize 知識庫。
- [GAP] change 文件已有 proposal、design、spec、tasks；目前 9 個本機／D1／Vectorize 任務已勾選，LINE secrets、正式 ingest token、知識查詢工具驗證與真人驗收仍未完成。
...
2. 先設定 LINE 兩個 secrets 與正式 Knowledge Ingest Token。
3. 不把本機測試或 dry-run 當成真人 LINE 驗收。
4. 每個未完成 task 先補行為證據，再勾選。

## 下一個動作

進入 P5 前置：設定正式 secrets，然後做 LINE Webhook 與知識查詢的真人驗收；不要把 smoke test 當正式驗收。
