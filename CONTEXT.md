...
status: P4 初版已施工並部署；Vectorize 與遠端匯入 smoke test 通過；待 LINE 憑證與真人驗收
---
...
- [QUESTION] LINE Channel Access Token、Channel Secret 尚未放進這個 Worker 的 secrets。（驗證方式：`wrangler secret list`）
...
- P5：尚未開始正式環境驗收；LINE 兩個 secrets、正式 Knowledge Ingest Token 與真人 LINE 測試仍是前置條件。

## 下一步

下一步設定 LINE Channel Access Token、Channel Secret 與正式 Knowledge Ingest Token；完成後做真人 LINE 驗收。
